...
void CreateImgPart(MandelSet* _this, std::mutex * locker, bool * exit)
{
	unsigned x = 0;
	unsigned y = 0;
	...
	while (x < _this->dscr.img_size.x && !exit) {
		locker->lock();
		locker->unlock();
...
MandelSet::MandelSet(dscr::Vec2<float> img_pos, 
					 dscr::Vec2<unsigned> img_size):
	RenderThreads<MandelSet>(4, this, CreateImgPart)
{
